...
/**
 * Tab component renders a single tabbed view for self-assessment subcategories,
 * including tiered questions, evidence, and comments. It handles tier navigation,
 * saving answers, and inviting team members.
 *
 * @component
 * @param {Object} props - Component props
 * @param {Object} props.cycle - Assessment cycle data
 * @param {string} props.name - Name of the main category
 * @param {Object} props.selectedSubCat - Currently selected subcategory object
 * @param {Function} props.setSelectedSubCat - Function to update the selected subcategory
 * @param {boolean} props.finish - Whether the assessment for this subcategory is finished
 * @param {Function} props.setFinish - Function to update the finish state
 * @param {Array} props.subCategories - List of subcategories
 * @returns {JSX.Element} A self-assessment interface with tiered tabs and inputs
 */
const Tab = ({
  cycle,
  name,
  ...
  // const [, setIVC1] = useState(0);
  // const [, setIVC2] = useState(0);
  // const [, setIVC3] = useState(0);
  ...
  /**
   * Advances to the next subcategory and resets finish/end flags accordingly.
   */
  const moveToNext = () => {
  ...
  /**
   * Moves to the previous active tier in the list.
   */
  const goBack = () => {
    setActiveTiers(tiers.filter((val) => val.sort_order === activeTiers.sort_order - 1)[0]);
  };
  /**
   * Submits the selected IVC answer for the active tier to the backend.
   * Handles tier progression and displays success/error toast messages.
   *
  ...
  const postAnswer = async () => {
    ...
    setBtnLoad(true);
    try {
      // Construct assessment response based on active tier and IVC choices
      const assessment = {
      ...
      if (ivcTierOneChoice === undefined) {
        ...
        await request(true).post(`/admin/ivc`, {
          'company-id': companyID[0],
          'category-id': selectedSubCat?.id,
          'tier': activeTiers?.tier_constant,
        ...
        if (activeTiers.sort_order !== 3 && tiers.length !== 1) {
          ...
          // Show success toast and progress to next tier or subcategory
          toast({
            status: 'success',
          ...
          // Clear previous choices from local storage
  ...
  /**
   * Fetches questions and tier data for the current subcategory.
   *
   * @param {number} type - Flag to determine loading state handling
   * @returns {Promise<void>}
   */
  ...
  return (
    <div
      ...
      style={{width: '100%'}}
    >
      {/* Render header with title and invite member button */}
      ...
      {/* Render FinishAnswer component if finished */}
      {finish ? (
        <FinishAnswer moveToNext={moveToNext} endOfSubCat={endOfSubCat} />
      ) : selectedSubCat?.id ? (
        <>
          {/* Render loading state or tiered tab interface */}
          {loading ? (
            ...
            <>
              {/* Render tab menu with tier names */}
              ...
              {/* Render subcategory title and description */}
              ...
              {/* Render Descriptor, Evidence, and Comments components */}
              {selectedSubCat?.id ? (
                <>
                  <Descriptor
                    cycle={cycle}
                    ...
                    setIVCTierOneChoice={(val) => setIVCTierOneChoice(val)}
                    ivcTierTwoChoice={ivcTierTwoChoice}
                    setIVCTierTwoChoice={(val) => setIVCTierTwoChoice(val)}
                    ivcTierThreeChoice={ivcTierThreeChoice}
                    setIVCTierThreeChoice={(val) => setIVCTierThreeChoice(val)}
                  />{' '}
                  <Evidence categoryId={selectedSubCat?.id} parentId={selectedSubCat?.parent_id} />
                  <Comments categoryId={selectedSubCat?.id} parentId={selectedSubCat?.parent_id} />
                </>
              ) : (
                ''
              )}
              {/* Render navigation buttons (Previous, Next/Save) */}
