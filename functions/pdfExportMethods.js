...
function downloadPDFWithPDFMake() {
  const tableHeaderText = [...document.querySelectorAll('#styledTable thead tr th')].map((thElement) => ({text: thElement.textContent, style: 'tableHeader'}));

  const tableRowCells = [...document.querySelectorAll('#styledTable tbody tr td')].map((tdElement) => ({text: tdElement.textContent, style: 'tableData'}));
